...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    // LOG(INFO) << layer_names()[i] << " (" << layers_[i]->type() << "): "
...
void Net<Dtype>::ForwardDebugInfo(const int layer_id) {
  for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
    ...
    for(int c=0; c<blob.channels(); ++c){
        ...
        for(int k=0; k<9; ++k){
            p = buff+strlen(buff);
            snprintf(p, 1024-strlen(buff), "%10f,", values[c*w*h+k]); 
        }
        LOG_IF(INFO, Caffe::root_solver()) << buff;
    }
    */
  }
  for (int param_id = 0; param_id < layers_[layer_id]->blobs().size();
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    DLOG(INFO) << "Copying source layer " << source_layer_name;
    auto target_layer = layers_[target_layer_id];
    vector<shared_ptr<Blob<Dtype> > >& target_blobs = target_layer->blobs();
    bool bn_related = false;
    if (strncmp(target_layer->type(), "ConvolutionBNFixed", 28) == 0
        || strncmp(target_layer->type(), "BatchNorm", 9) == 0) {
      // BN related layer should have other check
      CHECK(target_blobs.size() >= source_layer.blobs_size()
          && target_blobs.size() <= source_layer.blobs_size() + 1)
          << "Incompatible number of blobs for layer " << source_layer_name;
      bn_related = true;
    } else {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      if (bn_related && j >= source_layer.blobs_size()) {
        LOG(WARNING) << "No param " << j << " weights from layer '"
            << source_layer_name << "'.";
        continue;
      }
      if (!target_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
        ...
        auto& target_shape = target_blobs[j]->shape();
        if (bn_related &&
            target_blobs[j]->count() ==
                *max_element(target_shape.begin(),
                             target_shape.end())) {
          auto origin_source_shape = source_blob.shape_string();
          source_blob.Reshape(target_shape);
          target_blobs[j]->CopyFrom(source_blob);
          LOG(WARNING) << "Force copying param " << j
              << " weights from layer '" << source_layer_name
              << "'; shape mismatch.  Source param shape is "
              << origin_source_shape << "; target param shape is "
              << target_blobs[j]->shape_string() << ".";
          continue;
        }
