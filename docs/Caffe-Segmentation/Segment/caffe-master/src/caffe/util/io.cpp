...
cv::Mat ReadImageToCVMat(const string& filename, const int height,
    ...
    const bool is_color, const bool is_unchanged) {
  cv::Mat cv_img;
  int cv_read_flag;
  if (is_unchanged) {
...
cv::Mat ReadImageToCVMat(const string& filename,
    const int height, const int width, const bool is_color, const bool is_unchanged) {
  return ReadImageToCVMat(filename, height, width, 0, 0, is_color, is_unchanged);
/*
  cv::Mat cv_img;
...
cv::Mat ReadImageToCVMat(const string& filename,
    const bool is_color, const bool is_unchanged) {
  return ReadImageToCVMat(filename, 0, 0, is_color, is_unchanged);
}

cv::Mat ReadImageToCVMat(const string& filename) {
  return ReadImageToCVMat(filename, 0, 0, true, false);
}
...
bool ReadTxtToAnnotatedDatum(const string& labelfile, const int height,
    const int width, const std::map<string ,int>&  name_to_label ,AnnotatedDatum* anno_datum) {
  std::ifstream infile(labelfile.c_str());
  if (!infile.good()) {
    LOG(INFO) << "Cannot open " << labelfile;
    return false;
  }
  string label_tmp,image_name;
  float xmin, ymin, xmax, ymax;
  while (infile >> image_name >> label_tmp >> xmin >> ymin >> xmax >> ymax) {
    ...
    Annotation* anno = NULL;
    int label = name_to_label.find(label_tmp)->second;
    int instance_id = 0;
    bool found_group = false;
    for (int g = 0; g < anno_datum->annotation_group_size(); ++g) {
