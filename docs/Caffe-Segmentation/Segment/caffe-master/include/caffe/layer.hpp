...
class Layer {
 public:
  ...
  const Solver<Dtype>* solver() const { return solver_; }
  void SetSolver(const Solver<Dtype>* solver) { solver_ = solver; }

  int iter() const { return piter_ == nullptr ? 0 : *piter_; }
  void set_piter(const int* piter) { piter_ = piter; }
