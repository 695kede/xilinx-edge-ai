...
DEFINE_int32(height, 20, "Number of grids vertically.");
...
DEFINE_int32(resize_height, 640, "Height images are resized to");
...
int main(int argc, char** argv) {
  ...
  while (in >> filename) {
    ...
    lines.push_back(std::make_pair(jpgName, bbs));
    if (++count % 1000 == 0) {
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (!ReadImageToDatum(image_path, 0, 0, 0, is_color, "jpg", data.mutable_car_image_datum())) {
      continue;
    }
